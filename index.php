<?php
// 1. DATA ARTIKEL (Simulasi Database)
// Anda bisa menambah artikel baru di sini dengan ID yang berbeda
$articles = [
    1 => [
        "judul" => "Kenapa Kita Perlu Mengaktifkan Flight Mode Saat di Pesawat?",
        "kategori" => "Edukasi",
        "penulis" => "Admin Loka Ambon",
        "tanggal" => "13 Feb 2025",
        "waktu_baca" => "5 Menit",
        "gambar" => "assets/flight.jpeg",
        
    ],
    2 => [
        "judul" => "Izin Komunikasi Radio Perikanan (IKRAN) bagi Nelayan",
        "kategori" => "Layanan",
        "penulis" => "Admin Loka Ambon",
        "tanggal" => "13 Jan 2025",
        "waktu_baca" => "7 Menit",
        "gambar" => "assets/ikrap.webp",
        
    ],
    3 => [
        "judul" => "Tingkatkan Keselamatan Nelayan, Loka Monitor Ambon Gelar Bimtek SJJJ dan IKRAN",
        "kategori" => "Layanan",
        "penulis" => "Admin Loka Ambon",
        "tanggal" => "17 Nov 2025",
        "waktu_baca" => "7 Menit",
        "gambar" => "assets/bimtek.jpeg",
        
    ],
    4 => [
        "judul" => "Loka Monitor Ambon Perkuat Kualitas Pelayanan Publik melalui Sosialisasi dan Edukasi",
        "kategori" => "Edukasi",
        "penulis" => "Admin Loka Ambon",
        "tanggal" => "27 Agst 2025",
        "waktu_baca" => "7 Menit",
        "gambar" => "assets/edukasi.jpeg",
        
    ],
];

// 2. TANGKAP ID DARI URL
$id = isset($_GET['id']) ? (int)$_GET['id'] : 1; // Default ke ID 1 jika tidak ada parameter
$data = isset($articles[$id]) ? $articles[$id] : $articles[1]; // Jika ID tidak ditemukan, kembali ke 1
?>    
    ...
    <body>
        ...
        <ul class="dropdown__menu">
            ...
            <li>
                <a href="https://wbs.komdigi.go.id/" target="blank" class="dropdown__link">
        ...
        if (is_dir($dir)) {
            ...
            if (!empty($images)) {
                foreach ($images as $index => $file) {
                    ...
                    echo '<div class="hero__slide ' . $activeClass . '" style="background-image: linear-gradient(rgba(0,0,0,0.4), rgba(0,0,0,0.4)), url(\'' . $dir . $file . '\');"></div>';
                }
            }
        } else {
            echo '<div class="flex items-center justify-center h-full bg-gray-200">Folder foto tidak ditemukan</div>';
        }
        ?>
    </div>
</section>

        <main class="hero-section py-20 px-6">
        <div class="container mx-auto text-center max-w-4xl">
            <h2 class="text-xl md:text-2xl font-semibold text-gray-600 tracking-[0.2em] mb-12 uppercase reveal-left">
                Visi Loka Monitor Ambon
            </h2>
            
            <p class="text-2xl md:text-4xl font-light leading-[2] text-gray-700 italic px-4 reveal-left" style="transition-delay: 0.3s;">
                "Terwujudnya penggunaan spektrum frekuensi radio yang tertib, efisien dan bebas dari segala interferensi yang merugikan, melalui implementasi sistem pengendalian dan pengawasan yang profesional sesuai dengan teknologi komunikasi"
            </p>
        </div>
    </main>

    <main class="py-20 px-6 bg-gray-50 reveal-right">
        <div class="container mx-auto max-w-6xl">
            <div class="mb-12 border-b border-gray-200 pb-4">
                <h3 class="text-gray-500 uppercase tracking-widest text-sm font-bold mb-1">Informasi</h3>
                <h2 class="text-3xl font-black text-[#002B49] uppercase">Berita Terbaru</h2>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <?php 
                // Mengambil 4 artikel terbaru
                $latest_articles = array_slice($articles, 0, 4, true); 

                foreach ($latest_articles as $id_berita => $berita): 
                ?>
                <div onclick="location.href='baca-artikel.php?id=<?php echo $id_berita; ?>'" class="flex gap-4 group cursor-pointer bg-white p-3 rounded-xl hover:shadow-md transition-all border border-transparent hover:border-blue-100">
                    <div class="w-32 h-24 flex-shrink-0 overflow-hidden rounded-lg shadow-sm">
                        <img src="<?php echo $berita['gambar']; ?>" alt="Berita" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                    </div>
                    <div class="flex flex-col justify-center">
                        <h4 class="font-bold text-gray-800 leading-tight group-hover:text-blue-600 transition-colors uppercase text-sm mb-2 line-clamp-2">
                            <?php echo $berita['judul']; ?>
                        </h4>
                        <div class="flex items-center gap-4 text-[10px] text-gray-400 font-semibold">
                            <span class="flex items-center gap-1"><i class="ri-calendar-line"></i> <?php echo $berita['tanggal']; ?></span>
                            <span class="flex items-center gap-1"><i class="ri-time-line"></i> <?php echo $berita['waktu_baca']; ?></span>
                        </div>
                        <a href="baca-artikel.php?id=<?php echo $id_berita; ?>" class="mt-4 inline-flex items-center gap-2 text-blue-600 font-black text-[10px] uppercase tracking-tighter">
                            Selengkapnya <i class="ri-arrow-right-circle-fill text-sm"></i>
                        </a>
                    </div>
                </div>
                <?php endforeach; ?>
            ...
            <a href="artikel.php" class="inline-block bg-[#002B49] text-white px-8 py-3 rounded-full font-bold uppercase tracking-widest text-xs hover:bg-blue-600 transition-all shadow-lg">
                Lihat Semua Berita
            </a>
        </div>
        </div>
...
<section class="py-16 bg-white reveal-right" style="transition-delay: 0.2s;">
    <div class="container mx-auto max-w-6xl px-6">
        <div class="flex justify-between items-end mb-8 border-b border-gray-100 pb-4">
            <div>
                <h3 class="text-gray-400 uppercase tracking-widest text-xs font-bold mb-1">Galeri</h3>
                <h2 class="text-3xl font-black text-[#002B49] uppercase">Foto Kegiatan</h2>
            </div>
            <a href="foto.php" class="text-blue-600 font-bold text-sm flex items-center gap-2 hover:gap-4 transition-all uppercase tracking-tighter">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <?php
            // Konfigurasi folder sumber
            $albums = [
                "foto/lomba/" => "Lomba Digi Wave 2025",
                "foto/spm/"   => "Sumpah Pemuda 2025",
                "foto/r.nov2025/" => "Rekapan Kegiatan November 2025",
            ];

            $allDisplayImages = [];

            foreach ($albums as $dir => $title) {
                if (is_dir($dir)) {
                    // Ambil file gambar dari folder
                    $images = glob($dir . "*.{jpg,jpeg,png,gif,JPG,PNG,webp}", GLOB_BRACE);
                    if (!empty($images)) {
                        // Ambil maksimal 2 foto dari setiap folder
                        $selected = array_slice($images, 0, 2);
            ...
            // Loop untuk menampilkan ke dalam Grid
            foreach ($allDisplayImages as $index => $data): 
                // Foto pertama (index 0) dibuat besar (col-span-2)
                $isFirst = ($index === 0);
                $gridClass = $isFirst ? "col-span-2 row-span-2 shadow-lg" : "aspect-square shadow-md";
            ?>
                <a href="<?php echo $data['url']; ?>" 
                   class="glightbox <?php echo $gridClass; ?> relative group overflow-hidden rounded-2xl" 
                   data-gallery="gallery-home" 
                   data-glightbox="title: <?php echo $data['title']; ?>">
                    
                    <img src="<?php echo $data['url']; ?>" 
                         alt="<?php echo $data['title']; ?>" 
                         class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110">
                    
                    <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent <?php echo $isFirst ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'; ?> transition-opacity duration-300 flex items-end p-6">
                        <p class="text-white text-sm font-semibold"><?php echo $data['title']; ?></p>
                    </div>
...
<section class="py-16 bg-white reveal-right" style="transition-delay: 0.4s;"> ```
    <div class="container mx-auto max-w-6xl px-6">
        <div class="flex justify-between items-end mb-8 border-b border-gray-100 pb-4">
            <div>
                <h3 class="text-gray-400 uppercase tracking-widest text-xs font-bold mb-1">Galeri</h3>
                <h2 class="text-3xl font-black text-[#002B49] uppercase">Video</h2>
            </div>
            <a href="https://www.youtube.com/@lokamonitorsframbon8750" target="_blank" class="text-blue-600 font-bold text-sm flex items-center gap-2 hover:gap-4 transition-all uppercase tracking-tighter">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="md:col-span-3">
                <div class="relative rounded-[2rem] overflow-hidden shadow-2xl group aspect-video md:aspect-[21/9]">
                    <iframe class="w-full h-full" src="https://www.youtube.com/embed/VICqBzazTPE" title="Video Utama Loka Ambon" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
                    
                    <div class="absolute bottom-0 left-0 w-full p-8 bg-gradient-to-t from-black/80 to-transparent pointer-events-none transition-opacity group-hover:opacity-0">
                        <h3 class="text-white text-xl md:text-2xl font-bold leading-tight">
                            Berkarya Membangun Negeri
                        </h3>
                    </div>
                </div>
            </div>
            ...
            <div class="group">
                <div class="relative rounded-2xl overflow-hidden shadow-md aspect-video mb-3">
                    <iframe class="w-full h-full" src="https://www.youtube.com/embed/XCONZXJkWQg" frameborder="0" allowfullscreen></iframe>
                </div>
                <div class="flex gap-3 px-2">
                    <div class="w-8 h-8 rounded-full bg-blue-600 flex-shrink-0 flex items-center justify-center text-white">
                        <i class="ri-broadcast-line"></i>
                    </div>
                    <p class="text-xs font-bold text-gray-800 line-clamp-2 leading-snug group-hover:text-blue-600 transition-colors">
                        Loka Monitor SFR Ambon I Pengukuran Parameter Teknis
                    </p>
                </div>
            </div>
            ...
            <div class="group">
                <div class="relative rounded-2xl overflow-hidden shadow-md aspect-video mb-3">
                    <iframe class="w-full h-full" src="https://www.youtube.com/embed/hlyX6Mddu-E" frameborder="0" allowfullscreen></iframe>
                </div>
                <div class="flex gap-3 px-2">
                    <div class="w-8 h-8 rounded-full bg-blue-600 flex-shrink-0 flex items-center justify-center text-white">
                    ...
                    <p class="text-xs font-bold text-gray-800 line-clamp-2 leading-snug group-hover:text-blue-600 transition-colors">
                        Pengukuran Parameter Teknis Stasiun TV
                    </p>
                </div>
            </div>
            ...
            <div class="group">
                <div class="relative rounded-2xl overflow-hidden shadow-md aspect-video mb-3">
                    <iframe class="w-full h-full" src="https://www.youtube.com/embed/M0YydLljpUU" frameborder="0" allowfullscreen></iframe>
                </div>
                <div class="flex gap-3 px-2">
                    <div class="w-8 h-8 rounded-full bg-blue-600 flex-shrink-0 flex items-center justify-center text-white">
    ...
    // Mendaftarkan elemen reveal-left (yang sudah ada) dan reveal-right (yang baru)
    ...
    // Panggil fungsi statistik anda yang sudah ada
    window.addEventListener('load', () => {
    updateVisitorStats(); // Menjalankan statistik
    initRevealAnimation(); // Menjalankan animasi muncul
    if (typeof startHeroSlider === "function") startHeroSlider();
});
